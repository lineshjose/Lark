...
function lj_add_admin() {
	global $themename, $shortname, $options;
		...
		foreach ($options as $lj_default) 
		{
			if(get_option($lj_default['id'])=="")		 {		update_option($lj_default['id'],$lj_default['default']);		}
		}
...
<p>Thanks for downloading <strong><a href="http://linesh.com/projects/lark/"><?php echo $themename; ?></a></strong> by Linesh Jose. Hope you enjoy using it!</p>
	...
	?>
		...
		<h3>Current Saved Settings</h3>
			...
			<li>Google Plus: <strong><?php echo ucwords(get_option('lj_google_plus')); ?></strong></li>
				...
				<input type="hidden" name="action" value="lj_save" />
				</div>
			</div>
		</div>
	</div>
